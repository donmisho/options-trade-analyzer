...
def _build_trade_markdown(candidate) -> tuple[str, str]:
    ...
    structure_display = _display_structure(candidate.structure)
...
def _build_position_markdown(position: Position, latest_assessment) -> tuple[str, str]:
    ...
    structure_display = _display_structure(structure)
    trade_type = ts.get("trade_type", structure)
    trade_type_display = _display_structure(trade_type) if trade_type else structure_display
    ...
    lines = [
        ...
        f"**Current P&L:** {'+' if (position.current_pnl or 0) >= 0 else ''}{_fmt(position.current_pnl)}",
    ]
    ...
    lines.extend([
        ...
        "",
        f"- **Ticker:** {symbol}",
        f"- **Spread type:** {trade_type_display}",
        f"- **Strikes:** {strikes_label}",
        f"- **Expiration:** {_fmt_date(expiration)}",
